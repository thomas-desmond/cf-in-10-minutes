...
export async function POST(request: NextRequest) {
  ...
  const cachedName = await getRequestContext().env.cf_in_10_min.get(body.name)
  if (cachedName) {
    return Response.json(cachedName);
  }
  ...
  await getRequestContext().env.cf_in_10_min.put(body.name, response)
